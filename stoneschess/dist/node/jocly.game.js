		...
		Promise.all([SystemJS.import("jocly-xdview.js"), SystemJS.import("games/" + game.module + "/" + game.name + "-view.js")]).then(function (args) {
			...
			if (defaultViewOptions) {
				...
				for (var opt in optDefs) {
					if (typeof defaultViewOptions[optDefs[opt]] != "undefined") game[opt] = defaultViewOptions[optDefs[opt]];
				}
			}
	...
	if (gameData.initialBoard) {
		...
		if (!importResult.status) {
			...
			switch (importResult.error) {
				case 'parse':
					error = new Error("import failed: parse error");break;
				case 'unsupported':
					error = new Error("import failed: unsupported format");break;
			}
			throw error;
		}
